...
def receive_streaming_upload(handler):
    ...
    with tempfile.TemporaryDirectory() as temporary_directory:
        ...
        with tqdm(desc='Receiving data from remote host', total=total_size, dynamic_ncols=True, mininterval=1, unit='B', unit_scale=True) as progress_bar:
        ...
        if args.token:
            ...
            if token_target.value.decode(FORM_ENCODING) != args.token:
        ...
        # Upload successful
        handler.log_message('Upload of {} file(s) accepted'.format(len(directory_target.multipart_filenames)))
